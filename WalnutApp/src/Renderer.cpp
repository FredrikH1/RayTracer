...
glm::vec4 Renderer::PerPixel(uint32_t x, uint32_t y)
{
	...
	ray.Direction = glm::normalize(m_ActiveCamera->GetRayDirections()[x + y * m_FinalImage->GetWidth()]);
	...
	{	
		...
		if (material.dielectric && refractionRatio * glm::sqrt(1.0f - cosTheta * cosTheta) <= 1.0f && Utils::SchlickReflectance(abs(cosTheta), refractionRatio) < Utils::RandomFloat(seed))
		{
			...
			ray.Direction = glm::refract(glm::normalize(ray.Direction), payload.WorldNormal, refractionRatio);
		}
		else
		{
			...
			glm::vec3 diffuse = glm::normalize(payload.WorldNormal + material.Roughness * Utils::InUnitSphere(seed)); //Lambertian diffuse
...
#define HIT 0
	...
	for (size_t i = 0; i < m_ActiveScene->Triangles.size(); i++)
	{
		...
		glm::vec3 u1u2t = glm::inverse(mat) * (ray.Origin - triangle.Position);
	...
	return ClosestHit(ray, closestT, closestHitable);
